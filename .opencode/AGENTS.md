...
任何 Card* agent 在 `char/` 下创建或重命名产物文件后,**必须同时读取并更新 `char/index.yaml`**,将该文件注册为 `条目` 数组中的一项。

**具体要求:**

1. 写入产物文件后,立即用 Read 工具读取 `char/index.yaml`
2. 若 `index.yaml` 不存在,告知用户应先运行 CardIndex
3. 在 `条目` 数组中查找是否已有同名条目(按 `名称` 字段匹配):
   - **已存在** → 跳过(不重复添加)
   - **不存在** → 追加新条目,使用该文件类型对应的默认设置(见下表)
4. 写入更新后的 `char/index.yaml`

**各文件类型的条目默认设置:**

| 文件类型 | 激活策略 | 插入位置 | 顺序 | 递归 |
|---------|---------|---------|------|------|
| 世界设定 / 势力 / 主角设定 | 蓝灯 | 指定深度 / 系统 / 深度 4 | 100 | 不可被其他条目激活: false, 不可激活其他条目: false |
| 叙事风格 | 蓝灯 | 指定深度 / 系统 / 深度 0 | 100 | 同上 |
| 戏剧核心 | 蓝灯 | 角色定义之后 | 100 | 同上 |
| NPC | 蓝灯 | 角色定义之后 | 100 | 同上 |
| MVU 变量系列 | 蓝灯 | 指定深度 / 系统 / 深度 0 | 100 | 不可被其他条目激活: true, 不可激活其他条目: true |
| 第一条消息 | — | — | — | 属于 `第一条消息` 顶层字段,非条目 |

**重要例外:**

- **CardIndex** 自身不受此规则约束(它本就全权负责 index.yaml)
- **CardFirstMessage** 写入的文件属于 `第一条消息` 顶层字段而非 `条目` 数组,注册方式不同:确保 `第一条消息` 数组包含对应 `{ 文件: 第一条消息/0 }` 即可
- 路径写法:**不带** `.md` / `.yaml` 扩展名,相对 `char/` 根(例:`char/世界书/世界设定.md` → `文件: 世界书/世界设定`)

详细规则与条目格式参考手册见 `.opencode/instructions/worldbook-registry.md`。
...
- `worldbook-registry.md` — 世界书条目注册规则(写入文件后同步更新 index.yaml)
