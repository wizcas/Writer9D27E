...
请不要删除`示例`文件夹, AI 需要参考其中的代码; 但你可以在 `webpack.config.ts` 中将 54 行左右的 `{示例,src}/` 改为 `src/`
来避免打包它们.
...
- 推送到 `master` / `main`
  分支时自动触发, 将打包产物推送到一个独立的目标 CDN 仓库, 并在目标仓库打递增整数 tag, 从而让 jsdelivr 在 12 小时内更新缓存;
- 需要在本仓库 `Settings → Secrets and variables → Actions` 中配置以下内容后才会生效, 否则整个 job 会自动跳过:
  - **Variables**（非 Secrets）:
    - `RELEASE_REPO` — 目标仓库的完整名称, 如 `yourname/your-cdn-repo`
    - `RELEASE_PATH` — dist 内容在目标仓库中的存放路径, 如 `MyProject`
  - **Secrets**:
    - `RELEASE_TOKEN` — 有目标仓库写权限的 GitHub Personal Access Token

**`bundle.yaml`**（已禁用自动触发, 仅支持手动运行）

- 在本仓库的 `dist/` 文件夹中直接提交打包产物. 如果你不想使用独立 CDN 仓库, 可以手动运行此工作流作为替代.
